...
class CNN:
  def __init__(self, D, H, W, minibatch_size, num_classes, conv1_size, conv2_size, fc1_size):
    self.D = D
    self.H = H
    self.W = W
    ...
    self.W4 = tf.Variable(tf.truncated_normal([fc1_size, num_classes], stddev = 0.1))
    self.b4 = tf.Variable(tf.constant(0.1, shape=[num_classes]))
    self.H4 = tf.matmul(self.H3, self.W4) + self.b4

    self.cross_entropy = tf.reduce_mean(
      tf.nn.softmax_cross_entropy_with_logits(labels = self.y, logits = self.H4))

    self.train_step = tf.train.AdamOptimizer(1e-4).minimize(self.cross_entropy)
    self.correct_prediction = tf.equal(tf.argmax(self.H4, 1), tf.argmax(self.y, 1))
...
class TestCNN(unittest.TestCase):
  def test_overfit_random(self):
    ...
    for i in range(20):
      ...
      logging.info("step %d: accuracy = %f" % (i, accuracy))
