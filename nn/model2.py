...
class CNN2:
  def __init__(self, settings = {}):
    ...
    self.conv5_size = settings.get("conv5_size", 200)
    ...
    self.conv_W1 = tf.Variable(tf.truncated_normal((5, 3, 5, self.num_channels, self.conv1_size), stddev = 0.1))
    ...
    self.conv_W2 = tf.Variable(tf.truncated_normal((5, 3, 5, self.conv1_size, self.conv2_size), stddev = 0.1))
    ...
    self.conv_W3 = tf.Variable(tf.truncated_normal((5, 3, 5, self.conv2_size, self.conv3_size), stddev = 0.1))
    ...
    self.conv_W4 = tf.Variable(tf.truncated_normal((5, 3, 5, self.conv3_size, self.conv4_size), stddev = 0.1))
    ...
    self.conv_H6 = tf.nn.conv3d(self.conv_H5, self.conv_W6, (1, 1, 1, 1, 1), "SAME") + self.conv_b6
    logging.info("conv_H6 = %s" % str(self.conv_H6.shape))

    self.conv_last_flat = tf.reshape(self.conv_H6, (self.minibatch_size * self.DHW, self.num_classes))
    ...
    self.loss = self.cross_entropy + self.l2_reg * (tf.reduce_sum(self.conv_W1) +
                                                    tf.reduce_sum(self.conv_W2) +
                                                    tf.reduce_sum(self.conv_W3) +
                                                    tf.reduce_sum(self.conv_W4) +
                                                    tf.reduce_sum(self.conv_W5) +
                                                    tf.reduce_sum(self.conv_W6))
...
class TestCNN2(unittest.TestCase):
  ...
  def test_overfit(self):
    H = 32
    cnn = CNN2({"num_classes": 10, "minibatch_size": 1, "H": H, "W": H, "reg": 0})
  ...
  def test_minibatch_leak(self):
    H = 32
    cnn = CNN2({"num_classes": 2, "minibatch_size": 2, "H": H, "W": H, "reg": 0})
