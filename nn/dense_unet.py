...
class DenseUNet:
    ...
    def define_placeholders(self):
        ...
        self.X = tf.placeholder(
            tf.float32, shape=[self.S.batch_size] + image_shape + [self.S.image_channels])
        self.y = tf.placeholder(
            tf.uint8, shape=[self.S.batch_size] + image_shape + [1])
    ...
    def conv3d(self, inputs, output_channels, kernel_size, strides=[1, 1, 1, 1, 1]):
        ...
        W = self.weight_variable(
            [kernel_size, kernel_size, kernel_size, input_channels, output_channels], "W")
    ...
    def deconv3d(self, inputs, output_shape, kernel_size, strides=[1, 2, 2, 2, 1]):
        input_channels = int(inputs.get_shape()[-1])
        W = self.weight_variable(
            [kernel_size, kernel_size, kernel_size, input_channels, input_channels], "W")
    ...
    def composite(self, inputs, output_channels, kernel_size=None):
        if kernel_size is None:
            kernel_size = self.S.composite_kernel_size
    ...
    def dense(self, inputs, output_channels):
        outputs = self.conv3d(inputs, output_channels, 1) + \
            self.bias_variable([output_channels], "b")
        return outputs
    ...
    def add_layers(self):
        ...
        with tf.variable_scope("init"):
            outputs = self.conv3d(
                outputs, self.S.init_features, self.S.init_kernel_size)
            logging.info(str(outputs))
            ...
            with tf.variable_scope("down_block_%d" % layer):
                ...
                if layer != self.S.num_blocks - 1:
                    ...
                    outputs = self.bottleneck(
                        outputs, self.S.bottleneck_features)
                    logging.info(str(outputs))
        ...
        for layer in range(self.S.dense_layers):
            output_channels = self.S.num_classes if layer == self.S.dense_layers - \
                1 else int(outputs.get_shape()[-1])
    ...
    def add_softmax_loss(self):
        ...
        self.predictions = tf.reshape(
            predictions_flat, [self.S.batch_size, self.S.image_depth, self.S.image_height, self.S.image_width])
    ...
    def add_dice_loss(self):
        ...
        self.loss = -(2. * tf.reduce_sum(scores * y_flat2) + 1.) / (
            tf.reduce_sum(scores) + tf.reduce_sum(y_flat2) + 1.)
